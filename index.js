...
function CemberinAlani(yaricap, pisayisi) {
  let alan = Math.pow(yaricap, 2) * pi;
  return alan;
}
...
//3a çözümü
function kucuguBul(sayilarDizi) {
  enkucuk = sayilarDizi[0];
...
console.log("enkucuk: ", kucuguBul(sayilar));

function buyuguBul(sayilarDizi) {
  enbuyuk = sayilarDizi[0];
  for (let i = 0; i < sayilarDizi.length; i++) {
...
console.log("enbuyuk: ", buyuguBul(sayilar));
// 3b çözümü:
ucetambolunenler = [];
function arrayOlustur(sayilarDizi) {
  sayilarDizi.forEach((sayi) => {
    if (sayi % 3 === 0) {
